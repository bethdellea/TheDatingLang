...
def generate_cloud(text, maskIn): #max_font_size=40,
    cloud = WordCloud(background_color="white", relative_scaling=.5, font_path="C:/Windows/Fonts/ARBONNIE.ttf",
                      mask=maskIn, stopwords=STOPWORDS).generate(text)     #prev font: wensleydale_gothic_nbp.ttf
    plot.figure()                                                                   #hilarious with BMneco.TTF
    plot.imshow(cloud)
    plot.axis("off")
    plot.show()
